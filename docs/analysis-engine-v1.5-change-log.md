...
## v1.7.1 — Stress-Test Battery (traffic-aware catchments, prompt weights, named exclusions)

### 63. `backend-py/app/engine/canonical_archetypes.py` + `results.py` — traffic-aware drive catchments by default + free-flow honesty label
- **What:** `to_layers_dict()` now emits `trafficAware: True` on every drive catchment, so Pass-B refinement routes them through Google Routes with typical-traffic times (the capability existed since Phase 2 but no playbook enabled it). `_catchment_label()` in results.py labels a non-traffic-aware drive catchment "FREE-FLOW estimate — congested-city reach is substantially smaller" and a traffic-aware one "(typical traffic)", everywhere it appears.
- **Why (live-observed):** free-flow ORS isochrones drastically overstate reach in congested Indian metros — a free-flow "10-min drive" from Sector V reached Sealdah (~30 real minutes) and inflated a "10,360 competing supermarkets within 10-min drive" count. `schema_fingerprint` is unaffected (it doesn't include `trafficAware`), so archetype determinism is preserved; the executed spec's catchment genuinely changed, which is correct.
- **Risk:** low-medium — adds a bounded number of Google Routes calls per analysis (shortlist only, existing provider budget); disable per-archetype via the `trafficAware` flag if ever needed. **Rollback:** tag `rollback-pre-v1.7.1`.

### 64. `backend-py/app/engine/deterministic_planner.py` + `jobs.py` — prompt-stated weights, named-place exclusions, feasibility note
- **What:** `parse_prompt_weights()` reads "'Factor Name' (Weight: 0.7)" phrasing (percent normalized to fraction), `_match_layer_for_stated_name()` fuzzy-matches each to a spec layer by significant-token overlap, and `apply_deterministic_plan()` applies them (unmatched layers share the leftover mass; flagged `weightsAdjustedByUser`, `weightsSource=user_prompt`; unmatched names recorded in `promptWeightUnmatched`). `parse_named_exclusions()` requires BOTH an exclude-existing phrase and an ownership-context "branches/outlets/stores in <places>" phrase (so the business's own location never matches) and respects sentence boundaries; `apply_deterministic_plan()` writes `namedExclusions` (1.5 km buffer each). jobs.py geocodes each named exclusion and masks cells within the buffer (a non-geocodable place is disclosed as NOT enforced, never silently dropped), and appends the rent/floor-area feasibility note (system holds no rent-market/parcel-size data → shortlist tests the VERIFIABLE constraints only) plus the weight-audit / unmatched-weight notes.
- **Why:** canonical stress tests #8 (criteria weighting), #5 (exclusion & proximity), #3 (contradictory rent + arterial). The honest posture on #3 avoids fabricating rent-market knowledge the system does not hold.
- **Risk:** low — all additive, deterministic, disclosed; `weightsSource` now defaults via `.get()` so a prompt-weight source is not relabeled as registry. **Rollback:** tag `rollback-pre-v1.7.1`.

### 65. `backend-py/app/engine/results.py` — completes a v1.7.0 display detail
- **What:** `build_hex_grid()`'s per-factor `layerScores` (the map's per-factor choropleth) now applies the same `tx()` log transform as the composite (it was still linear), and `build_methodology()` spells out the log-space percentile stretch. Truthful map/score consistency under the log-normalization default.

### 66. Tests
- 8 new backend tests in `test_v164_map_and_coords.py`: `TestCriteriaWeightingPrompt` (parsing, percent-style, spec application with `user_prompt` source + 'Low Rent' disclosed unmatched), `TestExclusionProximityPrompt` (extraction, own-location non-match, spec reach with 1.5 km buffer), `TestIsochroneRealism` (drive catchments traffic-aware by default, free-flow labeled honestly).
- **629 backend passed** (621 + 8 new); frontend `tsc` clean, Vitest **75 passed** (backend-only change).
