...
## v1.8.0 — Screening & Investigation-Zone Product Contract (vNext)

Full audit + gap matrix: `docs/VNEXT_SCREENING_AND_INVESTIGATION_ZONE_AUDIT.md`.

### 68. `backend-py/app/engine/screening_contract.py` (new) + `services/jobs.py` wiring — screening vocabulary + next-validation actions
- **What:** a pure projection module. `apply_screening_verdicts()` maps the honesty-gated `investigationLabel` to per-zone `screeningVerdict` (Priority for the first recommended zone, Promising for later ones, Conditional/Low priority/Withheld for the rest) — it can never upgrade a verdict. `claim_level()` emits the run-level `claimLevel` (investigation_zone / uploaded_candidate). `build_zone_next_validation()` generates ≤6 action-phrased next-stage checks per zone from the run's ACTUAL unmet or screening-stage requirements: planner unsupported-constraint keys (rent/floor-area/zoning/availability/ownership), constraint-policy unverified names, uncomputed route checks on that zone, sparse competition coverage, degraded land checks, plus the standing zone→parcel step. Wired in jobs.py after the hard-constraint-verification safety cap (so demotions are final), wrapped in try/except (presentation must never break a run).
- **Why:** the taxonomy existed but spoke engine language; limitations were disclosed as statuses, not actions (vNext brief §5, §12: every limitation must lead to an action).
- **Risk:** minimal — additive keys, no scoring change. **Rollback:** tag `rollback-pre-v1.8.0`.

### 69. `models/spec.py` + `engine/scoring.py` + `engine/contracts.py` + `deterministic_planner.py` + `results.py` — target-band competition curve
- **What:** `Layer.scoringCurve: "monotonic"|"target_band"` (default monotonic = byte-identical old behaviour, test-locked). `curve_score()` / `normalize_0_1(curve=…)` implement an inverted-U over the normalized observed range (peak 0.35): zero ≈ 0.46, peak = 1.0, saturation = 0. Applied in Pass A, per-hex refined scoring, and the per-cell map layerScores — all three paths agree. `detect_competition_band()` fires only on explicit "less competition but NOT zero"-class phrasing (the canonical four-locality Kolkata prompt) and sets the curve on competition-family layers only. Disclosed in the factor justification, criteria payload (`scoringCurve`), the LLM explanation prompt (never describe a low band score as "no competition"), and the PDF criteria table ("Target band (moderate best)").
- **Why:** vNext brief §10 — a monotonic negative factor always gave the best score to zero observed competitors, contradicting briefs that want market validation.
- **Risk:** low — inactive unless the prompt asks; band bounds derive from the observed distribution (no magic counts); log-space Scoring Standard v1 inherited via `tx()`.

### 70. `engine/scoring.py` + `services/jobs.py` + `engine/results.py` — observed absence ≠ missing data
- **What:** `LayerScores.data_status ∈ {observed, observed_zero, unavailable}`, resolved from fetch outcomes (main-OSM failure / Places `_pp_src=="none"` → unavailable; successful-but-empty → observed_zero). Scoring is unchanged (`has_data` remains the only gate); the distinction drives the no-data criterion wording ("queried successfully but found ZERO features … never treated as an ideal score; validate locally" vs "provider failed … treat as unknown, not absent"), a distinct `observed-zero` evidence basis, `criteria.dataStatus`, and `dataQuality[].dataStatus` — and feeds sparse-competition next-validation actions.
- **Why:** vNext brief §3.2 / canonical prompt 7 (Nagpur): the two cases were conflated in prose notes and invisible to machines.

### 71. `engine/planner_lite.py` — spatial-scale classification
- **What:** `analysisIntelligence.spatialScale ∈ {site_or_block, micro_market, locality, city, metro_region, corridor}` — ordered deterministic rules over prompt text + study-area places (block/intersection keywords → site_or_block; region/outskirts/entire or ≥4 places → metro_region; phase/sector qualifiers → micro_market; corridors win outright). Disclosed in run notes; the frontend builds a methodology-comparison block from it.
- **Why:** vNext brief §9 — scale must affect methodology *disclosure*; canonical factors are never silently swapped.

### 72. `services/llm.py` — deterministic modification intent + new-brief context strip
- **What:** `MODIFY_SIGNAL` (recalculate/rerank/reweight/reverse/penalize/exclude/expand/narrow/compare/cap-at/…) marks imperative refinements: the spec is force-carried (same protection as affirmations) and the stage stays at framework. `NEW_ANALYSIS_RE` ("start a new analysis in Pune") strips `_CONTEXT_KEYS_ON_NEW_BRIEF` (namedExclusions, corridors, waterfront, radius override, adjusted-weight flags, exclusions, routeConstraints, studyArea, grid-choice flag) from the carried spec while keeping the business type.
- **Why:** vNext brief §8.1 — follow-up detection leaned on interrogatives; "Reverse the weights." has none. And "same business, new city" must not inherit stale spatial context.

### 73. Frontend — provisional reweighting, rank deltas, verify action, executive header, CTA (`src/services/screeningPresentation.ts` new; ResultsDrawer/App/MapView/resultNormalizer/types)
- **What:** (1) `computeRankDeltas()` compares the client-reweighted list against the backend-verified original: moved zones show ▲/▼ was #N; zones absent from the original are badged NEW — UNVERIFIED and inherit no evidence. (2) "Verify adjusted shortlist" re-runs the analysis via the normal job path with the user's weights written into the spec layers (`weightsAdjustedByUser: true` → audited). (3) Executive header (screened/eligible cells, top zone + verdict + fit, confidence, why-it-stands-out, critical next check, claim level + scale) — all computed. (4) Zone cards: verdict chip, evidence-backed reasons (direction/target-band aware), key risk, first next-validation; full list in the expander. (5) Zone-centroid wording on cards and map pins. (6) CTA: Request Detailed Site Validation (config.contactUrl) + prompt-free Copy analysis summary. (7) Methodology-comparison panel fed by `buildMethodologyComparison(prev, next)` (same session, same business only). (8) PDF: verdict strip, verdict-aware status badges, target-band label, per-zone Next-Stage Validation card, constraint-verification status table from `hardConstraintVerification`, CTA card. Normalizer sanitizes the new optional fields; older payloads render unchanged.

### 74. Tests
- New `tests/test_vnext_contract.py` — **43 tests**: target-band detection (incl. plain "less competition" stays monotonic), curve maths on all three scoring paths, planner application scoped to competition layers, observed-zero vs unavailable wording, verdict mapping (rank-aware, never-upgrade), next-validation generation/dedup/cap, sparse-competition detection, spatial-scale classes + determinism, follow-up signals (6 modification phrasings, 3 non-modifications), new-brief strip coverage, and the **nine-prompt canonical battery** (archetype + planning-fingerprint stability ×2 runs, dry-prompt waterfront negatives, riverside positive, supermarket rent/floor staging, dark-kitchen strict-route phrasing, Pune weight parse, Kolkata target-band, scale classes).
- New `src/__tests__/screeningPresentation.test.ts` — **15 tests** (reasons/risk/deltas/exec summary/comparison/copy summary).
- **679 backend passed** (636 + 43), **90 frontend passed** (75 + 15), `tsc --noEmit` clean, production build clean.
