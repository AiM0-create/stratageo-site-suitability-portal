...
class SpecV2(BaseModel):
    ...
    def validate_layers(self):
        ...
        # v1.5.2 — also include the user's RAW PROMPT (when carried on the spec):
        # the objective is now template-generated by the deterministic planner,
        # so water cues must be read from the customer's own words, which is
        # also simply the more truthful source.
        text = f"{self.objective} {self.businessType}".lower()
        if self.rawIntent is not None and getattr(self.rawIntent, "rawPrompt", None):
            text = f"{text} {self.rawIntent.rawPrompt}".lower()
